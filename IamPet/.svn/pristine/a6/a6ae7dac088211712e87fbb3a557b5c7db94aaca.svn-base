...
//				pstmt.setString(5, deliveryDto.getReceive_date());
		...
		{
			System.out.println("배송dao 시작 : " + order_code);
			DeliveryDTO deliveryDTO = null;
			Connection con = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			String findItem = "select delivery_no, order_code, member_id, delivery_status, send_date, receive_date from delivery where order_code = ?";
			System.out.println("배송dao 중간 : " + order_code);
			try
			{
				con = getConnection();
				pstmt = con.prepareStatement(findItem);
				pstmt.setInt(1, order_code);
				System.out.println("배송dao 검색 시작 : " + order_code);
				rs = pstmt.executeQuery();
				while(rs.next())
				{
					deliveryDTO = new DeliveryDTO(rs.getInt("delivery_no"), 
							  					  rs.getInt("order_code"), 
							  					  rs.getString("member_id"), 
							  					  rs.getString("delivery_status"), 
							  					  rs.getString("send_date"), 
							  					  rs.getString("receive_date"));
					System.out.println(rs.getInt("delivery_no")+", "+ 
		  					  rs.getInt("order_code")+", "+
		  					  rs.getString("member_id")+", "+
		  					  rs.getString("delivery_status")+", "+ 
		  					  rs.getString("send_date")+", "+
			...
			System.out.println("배송 dao검색 완료");
			return deliveryDTO;
		}
