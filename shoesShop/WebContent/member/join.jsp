...
function form_submit(){
	if(join.name.value==""){
		...
		join.id.focus();
		return false;
	}else if(join.pwd.value==""){
		alert("비밀번호입력해라");
		join.pwd.focus();
		return false;
	}else if(join.pwd.value!=join.repwd.value){
		alert("비밀번호가 같지않앙");
		return false;
	}else if(join.cell.value==""){
		alert("폰번입력해라");
		join.cell.focus();
		return false;
	}else if(join.addr1.value==""){
		alert("주소입력해라");
		join.addr1.focus();
		return false;
...
function checkId(){
	if(join.id.value==""){
		alert("아이디를 입력하고 중복검사하세요");
		join.id.focus();
	}else{
	window.open("member/checkId.jsp?id="+join.id.value);
	}
}
...
<td width="100%" valign="middle">
	...
	<form name="join" method="post" action="./member/joinok.jsp"
		...
		<table cellspacing="2" cellpadding="2" id="boxStyle" align="center">
			...
			<tr>
				<th>이름</th>
				<td><input type="text" name="name" value="" class="input"
			...
			<tr>
				<th>아이디</th>
				<td><input type="text" name="id" value="" class="input"
					maxlength="16" style="width: 133px;" /> <a
					href="javascript:checkId()"><img src="./images/member/check_id[1].gif" align="absmiddle"
						style="margin-top: 5px;" alt="아이디 중복확인" title="" /></a> <b></b></td>
			</tr>
			<tr>
				<th>
					<!--..K..Y…‥·
새 ·‥…S..Y..//--> 비밀번호
				...
				<td><input type="password" name="pwd" value="" class="input"
...
새 ·‥…S..Y..//--> 비밀번호확인
				...
				<td><input type="password" name="repwd" value="" class="input"
					maxlength="10" style="width: 133px;" /></td>
			</tr>
			<tr>
				<th>전화번호</th>
			...
			<tr>
				<th>주소</th>
				<td><input type="text" name="zip" value="" class="input"
					style="width: 70px;" /> <a
					href="javascript:zipSearch('joinFrm','zip','addr1','addr2')"><img
						src="./images/member/check_post[1].gif" alt="우편번호검색"
						align="absmiddle" style="margin-top: 5px;" title="" /></a><br /> <input
					type="text" name="addr1" value="" class="input"
					style="width: 300px;" maxlength="50" /><br />
			</tr>
			<tr>
				<th>이메일</th>
				<td><input type="text" name="email1" value="" class="input"
					style="width: 110px;" />&nbsp;@  
					<select name="email3">
